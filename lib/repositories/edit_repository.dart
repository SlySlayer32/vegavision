...
enum EditRequestSortField {
  createdAt,
  status,
}

// Sort fields for edit results
enum EditResultSortField {
  createdAt,
  processingTimeMs,
  status,
}
...
class EditRepositoryImpl implements EditRepository {
  ...
  Future<PaginatedResult<EditRequest>> getEditRequests({
    ...
    if (filter != null) {
      if (filter.status != null) {
        allRequests = allRequests.where((request) => request.status == filter.status).toList();
      }
      
      if (filter.startDate != null) {
        allRequests = allRequests.where((request) => request.createdAt.isAfter(filter.startDate!)).toList();
      }
      
      if (filter.endDate != null) {
        allRequests = allRequests.where((request) => request.createdAt.isBefore(filter.endDate!)).toList();
      }
    ...
    final List<EditRequest> pageItems = startIndex < total
  ...
  @override
  Future<void> updateEditRequestStatus(String id, EditRequestStatus status) async {
  ...
  Future<PaginatedResult<EditResult>> getEditResults({
    ...
    if (filter != null) {
      if (filter.status != null) {
        allResults = allResults.where((result) => result.status == filter.status).toList();
      }
      
      if (filter.startDate != null) {
        allResults = allResults.where((result) => result.createdAt.isAfter(filter.startDate!)).toList();
      }
      
      if (filter.endDate != null) {
        allResults = allResults.where((result) => result.createdAt.isBefore(filter.endDate!)).toList();
      }
    }
    ...
    allResults.sort((a, b) {
      ...
      switch (sortField) {
        ...
        case EditResultSortField.processingTimeMs:
          compareResult = (a.processingTimeMs ?? 0).compareTo(b.processingTimeMs ?? 0);
          break;
    ...
    // Get the items for the current page
    final List<EditResult> pageItems = startIndex < total
  ...
  Future<List<EditRequest>> createMultipleEditRequests(
    ...
    for (final data in requestData) {
      try {
        ...
        final additionalOptions = data['additionalOptions'] as Map<String, dynamic>?;
  ...
  Future<void> cleanupOldEditRequests(DateTime olderThan) async {
    ...
    final oldRequestIds = allRequests
  ...
  Future<void> cleanupFailedEditResults(DateTime olderThan) async {
    ...
    final failedResultIds = allResults
        .where((result) => 
