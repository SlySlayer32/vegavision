...
class ImageEditorViewModel extends ChangeNotifier {
  ...
  bool get hasValidInstruction =>
      _instruction.isNotEmpty && _instructionError == null;
  ...
  final UndoRedoStack<MarkerAction> _undoRedoStack =
      UndoRedoStack<MarkerAction>();
  ...
  Future<void> loadImage(String imageId) async {
    ...
    try {
      ...
      if (_selectedImage == null) {
        ...
        final ImageDimensions? dimensions = await _imageRepository
  ...
  void updateMarker(int index, Marker updatedMarker) {
    if (index >= 0 && index < _markers.length) {
      ...
      // Add to undo/redo stack
      _undoRedoStack.add(
        MarkerAction(type: ActionType.update, marker: oldMarker, index: index),
      );
